  ...
  var update = function () {
    ...
    var gEnter = g.enter().append("svg:g")
      .attr("class", function(d) {
        return (d.id == clientId) ? "node client" : "node client opponent";
      });
	...
	// Force redraw to update CSS on client side with change in game state
	var redraw = function(){
		d3.selectAll("circle").style("display", "none");
		d3.selectAll("circle").style("display", "block");
	};

	var updateCSS = setInterval(redraw, 1000);
